...
async function fetchDoubanJson(url) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(`Douban responded with ${response.status}`);
...
async function handleMetadata(response, url) {
  ...
  try {
    ...
    const endpoint = new URL("https://www.douban.com/j/search_suggest");
    endpoint.searchParams.set("q", query);
    const data = await fetchDoubanJson(endpoint);
