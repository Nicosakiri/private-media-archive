...
export type ProgressUnit = "page" | "minute" | "episode" | "chapter";
export type ProgressMode = "units" | "percent";
export type MangaProgressUnit = "" | "chapter" | "volume";
...
export type Entry = {
  ...
  mangaProgressUnit: MangaProgressUnit;
  totalUnits: number;
  totalVolumes: number;
  currentUnits: number;
  segmentCurrentUnits: number;
...
export function makeSegmentedProgressText(
  kind: "volume" | "episode" | "chapter",
  currentSegment: number,
  segmentCurrentUnits: number,
  segmentProgressPercent: number,
) {
  const segmentLabel =
    kind === "volume" ? "卷" : kind === "chapter" ? "话" : "集";
  const positionLabel = kind === "episode" ? "分钟" : "页";
