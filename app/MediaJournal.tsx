...
function isSegmentedEntry(entry: Entry) {
  return (
    ...
    (entry.mediaType === "book" &&
      (entry.bookCategory === "light_novel" ||
        (entry.bookCategory === "manga" && Boolean(entry.mangaProgressUnit))))
  );
}

function recalculateHistoricalNote(
  note: Note,
  entry: Pick<
    Entry,
    | "mediaType"
  ...
  const isManga =
    entry.mediaType === "book" &&
    entry.bookCategory === "manga" &&
    Boolean(entry.mangaProgressUnit);
  const isSegmented = entry.mediaType === "series" || isLightNovel || isManga;

  if (isSegmented) {
    const progressPercent = calculateSegmentedProgress(
      isLightNovel ? entry.totalVolumes : entry.totalUnits,
      isLightNovel ? note.volume : note.currentUnits,
      note.segmentProgressPercent,
    );
    return {
      ...note,
      progressPercent,
      status: deriveStatus(progressPercent, note.status === "abandoned"),
    };
  }

  const usesManualPercent =
    entry.mediaType === "book" &&
    (note.progressText.includes("%") ||
      (note.currentUnits === 0 &&
        note.progressPercent > 0 &&
        entry.progressMode === "percent"));
  if (usesManualPercent) {
    return {
      ...note,
      status: deriveStatus(
  ...
  const shouldRefreshText =
    note.currentUnits > 0 ||
    Boolean(note.progressText) ||
    (entry.mediaType === "movie" && entry.movieMode === "cinema");
  return {
    ...note,
    progressText: shouldRefreshText
      ? makeProgressText(
          entry.mediaType,
          entry.movieMode,
          entry.progressUnit,
...
export function MediaJournal() {
  ...
  const isMangaForm =
    form.mediaType === "book" &&
    form.bookCategory === "manga" &&
    Boolean(form.mangaProgressUnit);
  ...
  const formSegmentProgress = calculateInnerProgress(
    isMangaForm ? "percent" : form.progressMode,
  ...
  const recordProgress =
    selected && recordForm
      ? selected.mediaType === "book" &&
        ...
        : selected.mediaType === "book" &&
            selected.bookCategory === "manga" &&
            Boolean(selected.mangaProgressUnit)
          ? calculateSegmentedProgress(
              selected.totalUnits,
              recordCurrentUnits,
              recordSegmentProgress,
  ...
  function openEdit(entry: Entry) {
    ...
    setForm({
      ...
      progressMode: entry.progressMode || "units",
      mangaProgressUnit: entry.mangaProgressUnit || "",
  ...
  function changeBookCategory(bookCategory: BookCategory) {
    const isManga = bookCategory === "manga";
    const isLightNovel = bookCategory === "light_novel";
    setForm({
      ...form,
      bookCategory,
      progressUnit: "page",
      progressMode: isManga ? "percent" : "units",
      mangaProgressUnit: isManga ? form.mangaProgressUnit || "chapter" : "",
      totalUnits: "",
      totalVolumes: "",
      currentUnits: "",
      segmentCurrentUnits: "",
      ...
      volume: "",
      webFictionType:
        bookCategory === "web_fiction" ? form.webFictionType : "",
      danmeiTags: bookCategory === "web_fiction" ? form.danmeiTags : "",
    });
  }
  ...
  async function saveEntry(event: FormEvent) {
    ...
    try {
      ...
      const notes: Note[] = editing
        ? editing.notes.map((note) =>
            recalculateHistoricalNote(note, {
              ...
              progressMode:
                form.mediaType === "book" || form.mediaType === "series"
                  ...
                  : "units",
              mangaProgressUnit:
                form.mediaType === "book" && form.bookCategory === "manga"
                  ? form.mangaProgressUnit
                  : "",
              totalUnits,
      ...
      const progressText = segmented
        ? makeSegmentedProgressText(
            lightNovel
              ? "volume"
              : manga
                ? selected.mangaProgressUnit === "volume"
                  ? "volume"
                  : "chapter"
                : "episode",
            ...
            <form onSubmit={saveEntry}>
                      ...
                      {(["literary", "social_science", "textbook"] as BookCategory[]).map(
                        (category) => (
                          <button
                            ...
                            key={category}
                            onClick={() => changeBookCategory(category)}
                            type="button"
                          >
              ...
              {!isCinema && (
                <section className="automatic-progress">
                  ...
                  {form.mediaType === "book" && form.bookCategory === "manga" && (
                    <div className="progress-mode-switch" aria-label="漫画计量方式">
                      <button
                        className={form.mangaProgressUnit === "chapter" ? "active" : ""}
                        onClick={() =>
                          setForm({
                            ...form,
                            mangaProgressUnit: "chapter",
                      ...
                      <button
                        className={form.mangaProgressUnit === "volume" ? "active" : ""}
                        onClick={() =>
                          setForm({
                            ...form,
                            mangaProgressUnit: "volume",
                            progressMode: "percent",
                          })
                        }
                        type="button"
                  ...
                  {isLightNovelForm ? (
                    ...
                    isMangaForm ? (
                      <label className="field">
                        <span>
                          {form.mangaProgressUnit === "volume" ? "总卷数" : "总话数"}
                        </span>
                        <input
                          min="1"
                          onChange={(event) =>
        ...
        <div className="drawer-backdrop" role="presentation">
          ...
          >
            ...
            <dl className="detail-meta">
              ...
              {selected.bookCategory === "manga" &&
                selected.mangaProgressUnit &&
                selected.totalUnits > 0 && (
                  <div>
                    <dt>
                      {selected.mangaProgressUnit === "volume" ? "总卷数" : "总话数"}
                    </dt>
                    <dd>
                      共 {selected.totalUnits}{" "}
                      {selected.mangaProgressUnit === "volume" ? "卷" : "话"}
                    </dd>
                  </div>
                )}
                  ...
                  <section className="automatic-progress thought-progress-editor">
                    ...
                    selected.bookCategory === "light_novel" ? (
                      ...
                      <>
                        <label className="field">
                          <span>
                            当时看到第几{selected.mangaProgressUnit === "volume" ? "卷" : "话"}
                            （共 {selected.totalUnits || "?"} {selected.mangaProgressUnit === "volume" ? "卷" : "话"}）
                          </span>
                          <input
                        ...
                        <div className="field-row">
                          <label className="field">
                            <span>
                              当时本{selected.mangaProgressUnit === "volume" ? "卷" : "话"}进度（%）
                            </span>
                            <input
                              max="100"
                              min="0"
                ...
                <section className="automatic-progress record-progress">
                  ...
                  selected.bookCategory === "light_novel" ? (
                    ...
                    <>
                      <label className="field">
                        <span>
                          看到第几{selected.mangaProgressUnit === "volume" ? "卷" : "话"}
                          （共 {selected.totalUnits || "?"} {selected.mangaProgressUnit === "volume" ? "卷" : "话"}）
                        </span>
                        <input
                          autoFocus
                      ...
                      <div className="field-row">
                        <label className="field">
                          <span>
                            本{selected.mangaProgressUnit === "volume" ? "卷" : "话"}进度（%）
                          </span>
                          <input
                            max="100"
                            min="0"
